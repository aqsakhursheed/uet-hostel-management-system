<!-- # UET Hostel Management System

A first-semester Flask web application for hostel management. The project uses **Flask, HTML/CSS/JavaScript and JSON files** for data storage.

## Features

- Public hostel landing page
- Student account registration and login
- Hostel application submission
- Application status tracking
- Student complaint submission
- Admin dashboard
- Student, room, payment and staff management
- Application approval/rejection workflow
- Activity log and hostel settings
...
Set the admin credentials and session secret before starting:

```bash
export ADMIN_EMAIL="admin@uet.edu.pk"
...
This version intentionally keeps the original JSON-file architecture because it is a university/demo project. JSON files are suitable for demonstrating the project but are **not a production database**. On hosting platforms with ephemeral storage, changes to JSON data may be lost after a restart/redeploy. For a production version, migrate the data layer to PostgreSQL or another persistent database.

## Security changes made for deployment

- Admin-only API authorization
- Student-only complaint submission
- Students can only submit complaints for their own approved record
- Public application status lookup exposes only limited status information
- Admin credentials are loaded from environment variables
- Flask secret key is loaded from an environment variable
- Debug mode is disabled
- Gunicorn is used for production
- Broad wildcard CORS headers were removed
- Security response headers were added
- JSON writes are atomic to reduce partial-file corruption
- Input lengths/types are validated on important write endpoints
- Demo data contains no real CNICs, phone numbers, personal emails or real student identities

## Demo data

The JSON files contain fictional data only. Replace it with your own test data if needed, but do not upload real students' personal information to a public repository.
...
> This is a demo account created for project evaluation and portfolio demonstration. -->
